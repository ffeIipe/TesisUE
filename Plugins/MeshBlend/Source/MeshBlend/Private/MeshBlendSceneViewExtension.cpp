...
static TAutoConsoleVariable<int32> CVarMeshBlendQuality(TEXT("r.MeshBlend.Quality"), 3, TEXT("Low, Medium, High, Epic"), ECVF_Scalability);
...
static TAutoConsoleVariable<float> CVarMeshBlendNoiseFactor(TEXT("r.MeshBlend.NoiseFactor"), 0.5, TEXT(".."));
...
void FMeshBlendSceneViewExtension::PrePostProcessPass_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& View, const FPostProcessingInputs& Inputs)
{
	...
	static const auto CVarMeshBlendRunInEditor = IConsoleManager::Get().FindConsoleVariable(TEXT("r.MeshBlend.RunInEditor"));
	...
	if (World && !World->HasBegunPlay() && !CVarMeshBlendRunInEditor->GetBool())
	{
		return;
	}
#endif
                                                    ...
                                                    FSceneTextureShaderParameters SceneTextures)
{
	...
	MeshBlendData.NoiseFactor = FMath::Clamp(CVarMeshBlendNoiseFactor.GetValueOnRenderThread(), 0.0f, 1.0f);
...
#if UE_VERSION_OLDER_THAN(5, 4, 0)
#else
	if (Substrate::IsSubstrateEnabled() && ViewInfo.SubstrateViewData.SubstrateGlobalUniformParameters != nullptr)
	{
	...
	FPixelShaderUtils::AddFullscreenPass(GraphBuilder, GlobalShaderMap, FRDGEventName(TEXT("MeshBlend")), MeshBlendPixelShader, MeshBlendShaderParameters, PrimaryViewRect);
}
