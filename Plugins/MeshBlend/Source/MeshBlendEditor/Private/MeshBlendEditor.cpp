...
static TAutoConsoleVariable<bool> CVarMeshBlendCalculateOnPackaging(
	TEXT("r.MeshBlend.CalculateOnPackaging"),
	true,
	TEXT("Enable/Disable the calculating of blend IDs on packaging."));
...
void FMeshBlendEditorModule::TryResetMeshActivator()
{
	...
	AMeshBlendActivator::TryResetMeshActivator();
}
...
void FMeshBlendEditorModule::OnPreSaveWorld(UWorld* World, FObjectPreSaveContext ObjectPreSaveContext)
{
	...
	if (Processor == nullptr)
	{
		...
		Processor->AddToRoot();
		Processor->IsCooking = true;
	}

	const bool bCalculateBlendIDsOnPackaging = CVarMeshBlendCalculateOnPackaging.GetValueOnGameThread();
	...
	for (AActor* Actor : Level->Actors)
	{
		if (Actor && !Actor->IsEditorOnly())
		{
			if (FUAutoBlendHelper::DoesItBlend(Actor))
			{
				...
				if (bCalculateBlendIDsOnPackaging)
				{
					Processor->ActivateActor(Actor, FPlatformTime::Seconds() + 10);
				}
			}
			else
			{
